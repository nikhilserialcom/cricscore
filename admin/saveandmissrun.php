...
if($checkMatch)
{
    if($checkMatch['team1_id'] == $teamId)
    {
        foreach($checkMatch['team_1'] as &$filder)
        {
            ...
            {
                if($fildestatus == "run saved")
                {
                    $filder['run_saved'] += $missedRun; 
                }elseif($fildestatus == "run missed") {
                    $filder['run_missed'] += $missedRun;
                }
            }
        }
