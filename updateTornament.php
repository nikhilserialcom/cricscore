...
if (!isset($_SESSION['userId'])) {
    ...
    $torId = isset($_POST['torId']) ? new ObjectId($_POST['torId']) : '';
    ...
    $checkTorunament = $tournamentCollection->findOne($filter);
    if($checkTorunament){
        if($checkTorunament['userId'] == $_SESSION['userId']){
            ...
            $udateTournament = $tournamentCollection->updateOne($filter,$torData);
        
            if($udateTournament){
                $response = array(
                    'status_code' => 200,
                    'message' => "The tournament has been updated successfully"
                );
            }
            else{
                $response = array(
